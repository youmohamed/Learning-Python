...
number_of_guesses = 0
print("okey "+player_name+" ana youcef kon ja robot khamemt fi number between 1 to 20 aya ylzem tel9ah, 3endek 5 mo7awlat sinon takhser")
while number_of_guesses < 5:
    try:   # gestion des erreur
        guess = int(input())
        number_of_guesses += 1 # number_of_guess = number_of_guess + 1
        if guess < number:
            print("rak khrebtha zid tela3")
        elif guess > number:
            print("rak khrebtha zid 7abat")
        elif guess == number:
            break
    except ValueError:
